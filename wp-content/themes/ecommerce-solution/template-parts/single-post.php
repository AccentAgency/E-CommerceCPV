	...
	<?php if( get_theme_mod( 'ecommerce_solution_single_post_date',true) != '' || get_theme_mod( 'ecommerce_solution_single_post_author',true) != '' || get_theme_mod( 'ecommerce_solution_single_post_comment',true) != '') { ?>
		<div class="metabox p-2 p-3 mb-3">
	      	<?php if( get_theme_mod( 'ecommerce_solution_single_post_date',true) != '') { ?>
	        	<span class="entry-date mr-2"><i class="<?php echo esc_attr(get_theme_mod('ecommerce_solution_single_post_date_icon','far fa-calendar-alt')); ?> mr-1"></i><a href="<?php echo esc_url( get_day_link( $archive_year, $archive_month, $archive_day)); ?>"><?php echo esc_html( get_the_date() ); ?><span class="screen-reader-text"><?php echo esc_html( get_the_date() ); ?></span></a></span><?php echo esc_html( get_theme_mod('ecommerce_solution_single_post_meta_seperator') ); ?>
	      	<?php }?>
	      	<?php if( get_theme_mod( 'ecommerce_solution_single_post_author',true) != '') { ?>
	        	<span class="entry-author mr-2"><i class="<?php echo esc_attr(get_theme_mod('ecommerce_solution_single_post_author_icon','fas fa-user')); ?> mr-1"></i><a href="<?php echo esc_url( get_author_posts_url( get_the_author_meta( 'ID' )) ); ?>"><?php the_author(); ?><span class="screen-reader-text"><?php the_title(); ?></span></a></span><?php echo esc_html( get_theme_mod('ecommerce_solution_single_post_meta_seperator') ); ?>
	      	<?php }?>
	      	<?php if( get_theme_mod( 'ecommerce_solution_single_post_comment',true) != '') { ?>
	        	<span class="entry-comments mr-2"><i class="<?php echo esc_attr(get_theme_mod('ecommerce_solution_single_post_comment_icon','fas fa-comments')); ?> mr-1"></i> <?php comments_number( __('0 Comment', 'ecommerce-solution'), __('0 Comments', 'ecommerce-solution'), __('% Comments', 'ecommerce-solution') ); ?></span>
	      	<?php }?>
	      	<?php if( get_theme_mod( 'ecommerce_solution_single_post_time',false) != '' ) { ?>
          		<span class="entry-time mr-2"><i class="<?php echo esc_attr(get_theme_mod('ecommerce_solution_single_post_time_icon','fas fa-clock')); ?> mr-1"></i> <?php echo esc_html( get_the_time() ); ?></span>
	        <?php }?>
	    </div>
	<?php }?>
